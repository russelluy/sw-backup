...
// thanks to Jeremy at http://clarktech.no-ip.com for the tips
	if (!empty($_SERVER['SCRIPT_FILENAME']) && 'comments.php' == basename($_SERVER['SCRIPT_FILENAME']))
		die (__('Please do not load this page directly. Thanks!', 'safewayhealth'));
if (function_exists('post_password_required')) 
	{
	if ( post_password_required() ) 
		{ ?>
		<p class="nocomments"><?php __('This post is password protected. Enter the password to view comments.', 'safewayhealth');?></p>
		<?php 
		return;
		}
	} else 
...
if ( have_comments() ) : ?>
	...
	($count !== 1) ? $txt = __('Comments for this entry', 'safewayhealth') : $txt = __('Comment for this entry', 'safewayhealth'); ?>
	...
	($countp !== 1) ? $txtp = __('Trackbacks / Pingbacks for this entry ', 'safewayhealth') : $txtp = __('Trackback or Pingback for this entry', 'safewayhealth'); ?>
...
<?php else : // this is displayed if there are no comments so far ?>
	...
	else : ?><p class="nocomments"><?php echo __('Comments are closed.', 'safewayhealth');?></p>
	<?php endif;
endif;
endif;
?>
<?php if ('open' == $post->comment_status) : ?>
<div class="related-comment-title"><h2><?php echo __('Leave a comment', 'safewayhealth');?></h2></div><br />
...
<fieldset>
<?php if ( $user_ID ) : ?>
<p>Logged in as <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo wp_logout_url(get_permalink());?>" title="Log out of this account"><?php echo __('Logout &raquo;', 'safewayhealth');?></a></p>
<?php else : ?>
			<label for="author"><?php echo __('Name ', 'safewayhealth');?><?php if ($req) echo "<span>(required)</span>"; ?></label>
			...
			<label for="email"><?php echo __('Mail', 'safewayhealth');?> <?php if ($req) echo "<span>(required)</span>"; ?></label>
      ...
      <label for="url"><?php echo __('Website', 'safewayhealth');?></label>
...
<!--<p><small><strong>XHTML:</strong> You can use these tags: <?php echo allowed_tags(); ?></small></p>-->
			<label for="comment"><?php echo __('Comment', 'safewayhealth');?> <span><?php echo __('(required)');?></span></label>
