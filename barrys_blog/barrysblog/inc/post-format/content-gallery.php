	...
	<div class="entry-image entry-slider">
		<div class="entry-slides<?php echo ((count($images) > 1) ? ' nivo-slider' : '') ?>">
			<?php foreach( $images as $image ) :  ?>
				<a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__('Permalink to %s', 'barry'), the_title_attribute('echo=0') ); ?>" rel="bookmark"><?php echo wp_get_attachment_image($image->ID, 'gallery-large'); ?></a>
			<?php endforeach; ?>
		</div>
	</div>
		...
		<div class="entry-content">
			...
			<?php 
				if(of_get_option('enable_excerpt')){
					...
					echo str_replace(']]>', ']]&gt;', apply_filters('the_content', preg_replace('#<img[^>]*>#i', '', get_the_content(__('Continue reading...', 'barry')))));
				}
			?>						
