...
class bl_tabs extends WP_Widget
{
  function bl_tabs(){
    ...
    $this->WP_Widget('bl_tabs', 'barry Tabs', $widget_ops);
  }
  ...
  function widget($args, $instance){
    ...
    <?php echo !empty($instance['title']) ? '<h3 class="widget-head">'.$instance['title'].'</h3>' : '' ?>
	    <ul class="nav nav-tabs" id="bl_side_tabs">
		    <?php if($show_recent_posts){ ?><li class="active"><a href="#bl_side_posts"><?php _e('Posts', 'barry'); ?></a></li><?php } ?>
		    <?php if($show_recent_comments){ ?><li><a href="#bl_side_comments"><?php _e('Comments', 'barry'); ?></a></li><?php } ?>
		    <?php if($show_tags){ ?><li><a href="#bl_side_tags"><?php _e('Tags', 'barry'); ?></a></li><?php } ?>
	    </ul>
