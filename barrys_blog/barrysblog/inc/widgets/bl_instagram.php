...
class bl_instagram extends WP_Widget
{
  function bl_instagram(){
    ...
    $this->WP_Widget('bl_instagram', 'barry Instagram', $widget_ops);
  }
  ...
  <ol>
    <li>Attain your access token and user id <a href="https://api.instagram.com/oauth/authorize/?client_id=e802ca96dd27470f9ef0271bc9f0e6a3&redirect_uri=http://www.barry.is/&response_type=code" target="_blank">by clicking here</a></li>
      ...
      <div class="widget-body">
	    <?php
	    		...
	    		elseif(empty($instance['access_token'])){
				    ...
				    if(($cache = wp_cache_get('bl_instagram', 'barry')) === false){
						...
						if(!isset($posts->meta->error_message) or !isset($user->meta->error_message)){
					        ...
					        wp_cache_set( 'bl_instagram', array('posts' => $posts_data, 'user' => $user_data), 'barry', 21600);
						}else{
							wp_cache_delete( 'bl_instagram', 'barry' );
						}
						...
						<ul class="instagram-header">	
							<li>
								<small class="muted"><?php _e('Followers', 'barry'); ?></small>
								<p><?php echo $user->data->counts->followed_by ?></p>
							</li>	
							<li>
								<small class="muted"><?php _e('Following', 'barry'); ?></small>
								<p><?php echo $user->data->counts->follows ?></p>
							</li>	
							<li>
								<small class="muted"><?php _e('Images', 'barry'); ?></small>
