...
function mytheme_comment($comment, $args, $depth) {
   $GLOBALS['comment'] = $comment; ?>
		...
		<div class="clear"></div>
  		...
  		<p><?php echo __('Your comment is awaiting moderation.','safewaytheme');?></p>
  		<?php endif; ?>
  		<?php comment_text() ?>
  		<div class="clear"></div>
      <div class="reply">
...
function get_related_post() {
  ...
  echo '<div id="related-post-title"><h4>'.__('Related Posts','safewaytheme').'</h4></div>';                                         
  $original_post = $post;
  ...
  if ($tags) {
    ...
    echo "<p>".__('There is no related post.','safewaytheme')."</p>";
  }
  echo '</div>';
...
function safewaytheme_topmenu_pages() {
...
function safewaytheme_nav_menu_args( $args = '' ) {
	$args['container'] = false;
	return $args;
} 
add_filter( 'wp_nav_menu_args', 'safewaytheme_nav_menu_args' );
...
function safewaytheme_featuredproject($portocat,$num=2,$title) { 
  global $post;
...
function safewaytheme_serviceslist($service_page,$title) { 
  global $post;
...
load_theme_textdomain( 'safewaytheme', TEMPLATEPATH . '/languages' );
