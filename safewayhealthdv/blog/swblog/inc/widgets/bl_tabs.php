...
class bl_tabs extends WP_Widget
{
  function bl_tabs(){
    ...
    $this->WP_Widget('bl_tabs', 'swblog Tabs', $widget_ops);
  }
  ...
  function widget($args, $instance){
	...
	echo preg_replace('/class="[^"]*"/','class="box bl_tabs"', $before_widget);
    ?>
    <?php echo !empty($instance['title']) ? '<h3 class="widget-head">'.$instance['title'].'</h3>' : '' ?>
	    <ul class="nav nav-tabs" id="bl_side_tabs">
		    <?php if($show_recent_posts){ ?><li class="active"><a href="#bl_side_posts"><?php _e('Posts', 'swblog'); ?></a></li><?php } ?>
		    <?php if($show_recent_comments){ ?><li><a href="#bl_side_comments"><?php _e('Comments', 'swblog'); ?></a></li><?php } ?>
		    <?php if($show_tags){ ?><li><a href="#bl_side_tags"><?php _e('Tags', 'swblog'); ?></a></li><?php } ?>
	    </ul>
					    ...
					    <?php
					        ...
					        <li>
					        	<?php if ( $images = get_children(array('post_parent' => get_the_ID(), 'post_type' => 'attachment', 'post_mime_type' => 'image' ))){ 
									...
									echo '<a class="tab_attachment" href="'. get_permalink().'"><div class="tab_icon tab_'.$post_format.'"></div></a>';
								} 
					        	echo '<div class="tab_text"><a href="'. get_permalink().'">'.get_the_title().'</a></div>'; 
					        	?>
					        </li>
					        <?php endforeach; ?>
					</ul>
