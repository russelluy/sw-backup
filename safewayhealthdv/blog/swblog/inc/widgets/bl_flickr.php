...
{ 
	...
	function bl_flickr() {
		...
		// $this->WP_Widget( 'bl_flickr', 'swblog Flickr', $widget_ops, $control_ops );

	    $widget_ops = array('classname' => 'bl_flickr', 'description' => 'Displays recent instagram images in a widget' );
	    $this->WP_Widget('bl_flickr', 'swblog Flickr', $widget_ops);		
	}
	...
	function form($instance) {
	  ...
	  <ol>
		...
		<p>
			...
			<input class="widefat" id="<?php echo $this->get_field_id( 'num_img' ); ?>" name="<?php echo $this->get_field_name( 'num_img' ); ?>" value="<?php echo $instance['num_img']; ?>" />
		</p>
		<?php
	}
	...
	function widget($args, $instance) {
		...
		echo preg_replace('/class="[^"]*"/','class="box bl_flickr"', $before_widget);
		echo !empty($title) ? '<h3 class="widget-head"><i class="icon-flickr-circled"></i> '.$title.'</h3>' : '';
