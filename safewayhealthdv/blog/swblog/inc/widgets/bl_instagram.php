...
class bl_instagram extends WP_Widget
{
  function bl_instagram(){
    $widget_ops = array('classname' => 'bl_instagram', 'description' => 'Displays recent instagram images in a widget' );
    $this->WP_Widget('bl_instagram', 'swblog Instagram', $widget_ops);
  }
  ...
  <ol>
    <li>Attain your access token and user id <a href="https://api.instagram.com/oauth/authorize/?client_id=e802ca96dd27470f9ef0271bc9f0e6a3&redirect_uri=http://www.swblog.is/&response_type=code" target="_blank">by clicking here</a></li>
  ...
  function widget($args, $instance){
	...
	echo preg_replace('/class="[^"]*"/','class="box bl_instagram"', $before_widget);
    ?>
      ...
      <div class="widget-body">
	    <?php
	    		...
	    		elseif(empty($instance['access_token'])){
				    ...
				    if(($cache = wp_cache_get('bl_instagram', 'swblog')) === false){
						...
						if(!isset($posts->meta->error_message) or !isset($user->meta->error_message)){
					        ...
					        wp_cache_set( 'bl_instagram', array('posts' => $posts_data, 'user' => $user_data), 'swblog', 21600);
						}else{
							wp_cache_delete( 'bl_instagram', 'swblog' );
						}
						...
						<ul class="instagram-header">	
							<li>
								<small class="muted"><?php _e('Followers', 'swblog'); ?></small>
								<p><?php echo $user->data->counts->followed_by ?></p>
							</li>	
							<li>
								<small class="muted"><?php _e('Following', 'swblog'); ?></small>
								<p><?php echo $user->data->counts->follows ?></p>
							</li>	
							<li>
								<small class="muted"><?php _e('Images', 'swblog'); ?></small>
						...
						<ul class="instagram-interactions">
							<li class="instagram-comments"><i class="icon-comment"></i> <span><?php echo $interactions[0]['comments'] ?></span></li>
							<li class="instagram-likes"><i class="icon-heart"></i> <span><?php echo $interactions[0]['likes'] ?></span></li>
						</ul>
						<div class="instagram_link visible-desktop"><a href="<?php echo $interactions[0]['link'] ?>" target="_blank"><i class="icon-link"></i></a></div>
						<div class="left_arrow instagram_arrow visible-desktop"><i class="icon-left-open"></i></div>
						<div class="right_arrow instagram_arrow visible-desktop"><i class="icon-right-open"></i></div>
						<?php } ?>
			<?php } ?>
      </div>
