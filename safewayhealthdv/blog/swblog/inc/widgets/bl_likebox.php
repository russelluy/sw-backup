...
class bl_likebox extends WP_Widget
{
  function bl_likebox(){
    ...
    $this->WP_Widget('bl_likebox', 'swblog Likebox', $widget_ops);
  }
  ...
  function widget($args, $instance){
    ...
    echo preg_replace('/class="[^"]*"/','class="box bl_likebox"', $before_widget);
    ?>
      <?php echo !empty($instance['title']) ? '<h3 class="widget-head"><i class="icon-facebook"></i> '.$instance['title'].'</h3>' : '' ?>
      <div class="widget-body" id="bl_likebox">
        <iframe src="//www.facebook.com/plugins/likebox.php?href=<?php echo urlencode($instance['url']) ?>&amp;width=270&amp;height=558&amp;show_faces=<?php echo ($instance['show_faces']) ? 'true' : 'false'; ?>&amp;colorscheme=<?php echo $instance['color_scheme']; ?>&amp;stream=<?php echo ($instance['show_stream']) ? 'true' : 'false'; ?>&amp;show_border=false&amp;header=false" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:100%; height:558px;" allowTransparency="true"></iframe>
      </div>
    <?php
    echo $after_widget;
  }
