<?php
	...
	// load language
	load_theme_textdomain( 'swblog', get_template_directory() . '/inc/lang' );

	// Define the version so we can easily replace it throughout the theme
	define( 'swblog_VERSION', 1.42 );
	...
	add_action( 'after_setup_theme', 'swblog_register_nav_menu' );
	function swblog_register_nav_menu() {
		register_nav_menus( 
			array(
	...
	add_action('tgmpa_register', 'swblog_register_required_plugins');
	...
	function swblog_register_required_plugins() {
		$plugins = array(
			array(
		 ...
		 */
		$config = array(
			'domain'       		=> 'swblog',         	// Text domain - likely want to be the same as your theme.
			...
			'strings'      		=> array(
				'page_title'                       			=> __( 'Install Required Plugins', 'swblog' ),
				'menu_title'                       			=> __( 'Install Plugins', 'swblog' ),
				'installing'                       			=> __( 'Installing Plugin: %s', 'swblog' ), // %1$s = plugin name
				'oops'                             			=> __( 'Something went wrong with the plugin API.', 'swblog' ),
				...
				'return'                           			=> __( 'Return to Required Plugins Installer', 'swblog' ),
				'plugin_activated'                 			=> __( 'Plugin activated successfully.', 'swblog' ),
				'complete' 									=> __( 'All plugins installed and activated successfully. %s', 'swblog' ), // %1$s = dashboard link
	...
	function swblog_assets()  { 
		...
		// add theme css
		wp_enqueue_style( 'swblog-bootstrap', get_template_directory_uri() . '/assets/css/bootstrap.min.css' );
		wp_enqueue_style( 'swblog-style', get_template_directory_uri() . '/style.css' );
		// if disable responsive
		if(!$disable_responsive){
			wp_enqueue_style( 'swblog-responsive', get_template_directory_uri() . '/assets/css/style-responsive.css' );
		}
		wp_enqueue_style( 'swblog-fontello', get_template_directory_uri() . '/assets/css/fontello.css' );
		wp_enqueue_style( 'swblog-nivo', get_template_directory_uri() . '/assets/css/nivo-slider.css' );
		wp_enqueue_style( 'swblog-magnific', get_template_directory_uri() . '/assets/css/magnific-popup.css' );
		wp_enqueue_style( 'swblog-snippet', get_template_directory_uri() . '/assets/css/jquery.snippet.min.css' );

			
		// add theme scripts
		wp_enqueue_script( 'swblog-jquery-ui', $protocol.'://ajax.googleapis.com/ajax/libs/jqueryui/1.10.3/jquery-ui.min.js', array('jquery'), swblog_VERSION, true );
		wp_enqueue_script( 'swblog-snippet', get_template_directory_uri() . '/assets/js/jquery.snippet.min.js', array('jquery'), swblog_VERSION, true );
		wp_enqueue_script( 'swblog-theme', get_template_directory_uri() . '/assets/js/theme.min.js', array('jquery'), swblog_VERSION, true );
		wp_enqueue_script( 'swblog-nivo', get_template_directory_uri() . '/assets/js/jquery.nivo.slider.pack.js', array(), swblog_VERSION, true );
		wp_enqueue_script( 'swblog-timeago', get_template_directory_uri() . '/assets/js/jquery.timeago.js', array('jquery'), swblog_VERSION, true );
		wp_enqueue_script( 'swblog-bootstrap', get_template_directory_uri() . '/assets/js/bootstrap.min.js', array('jquery'), swblog_VERSION, true );
		wp_enqueue_script( 'swblog-magnific', get_template_directory_uri() . '/assets/js/jquery.magnific-popup.js', array('jquery'), swblog_VERSION, true );
		...
		wp_enqueue_style( 'swblog-googlefonts', $protocol.'://fonts.googleapis.com/css?family='.$text_font.'|'.$heading_font. (!empty($subset_array) ? '&subset='.implode(',', $subset_array) : '')  );	

	    if ( is_singular() && get_option( 'thread_comments' ) )
	        wp_enqueue_script( 'comment-reply' );			
	}
	add_action( 'wp_enqueue_scripts', 'swblog_assets' ); // Register this fxn and allow Wordpress to call it automatcally in the header
	...
	{  
	     ...
	     if(1 != $pages)
	     {
			...
			echo get_previous_posts_link( '<i class="icon-left-open"></i> '.__('Previous Page', 'swblog') );
			...
			echo get_next_posts_link( __('Next Page', 'swblog').' <i class="icon-right-open"></i>' );
	        echo "</div>\n";
	     }
	}
	 ...
	 */
	if ( !function_exists( 'swblog_comment' ) ){
		function swblog_comment( $comment, $args, $depth ) {
		    ...
		    <li class="post pingback">
		        <p><?php _e( 'Pingback:', 'swblog' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '(Edit)', 'swblog' ), ' ' ); ?></p>
		    <?php
		            break;
		        default :
		    ?>
		        ...
		        <article id="comment-<?php comment_ID(); ?>" class="comment">
		            <div>
		                <div class="comment-author">
		                    ...
		                    <?php printf( __( '%s', 'swblog' ), sprintf( '<cite class="commenter">%s</cite>', get_comment_author_link() ) ); ?>
		                	...
		                	&nbsp;&nbsp;<span class="label label-success"><?php _e('Author', 'swblog'); ?></span>
		                	<?php } ?>
		                </div><!-- .comment-author .vcard -->
		                <?php if ( $comment->comment_approved == '0' ) : ?>
		                    <em><?php _e( 'Your comment is awaiting moderation.', 'swblog' ); ?></em>
		                    <br />
		                <?php endif; ?>
		            </div>
		            ...
		            <div class="reply">
		                ...
		                <?php edit_comment_link( __( '(Edit)', 'swblog' ), '&nbsp;&nbsp;' ); ?>
		            </div><!-- .reply -->
		        </article><!-- #comment-## -->
	...
	function excerpt_read_more_link($output) {
	 global $post;
	 return $output . '<p><a class="more-link" href="'. get_permalink($post->ID) . '"> '.__('Continue reading...', 'swblog').'</a></p>';
	}
